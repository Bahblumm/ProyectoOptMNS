...
Niter = 100; % Veces que intentamos un cambio aleatorio de plato o piñon
for i = 1:5 
...
for  j = 1:Niter
   p = randi(8); % eleccion aleatoria de plato/pinon a cambiar
    ...
    if p<4 % p = 1,2,3 se traduce a cambiar el plato numero 1,2,3
        ...
        if isempty(N(N==Np)) % comprobamos que Np no sea igual a algun otro plato
            ...
            if length(M26(M26< M(7) & M26> M(1))) == 5 % comprobamos que M2-M6 estan comprendidos entre los nuevos M1,M7
                ...
                if normrAux < bici.normr % Actualiza la estructura si encontramos una norma mejor 
                    bici.platos = N;
                    bici.pinones = M;
                    bici.desarrollos = desarrollos;
                    bici.normr = normrAux;
        ...
        Mp = randi(M(7)-M(1)-1)+M(1); % randi([M(1)+1, M(7)-1])
        ...
        if isempty(M(M==Mp)) % nos aseguramos de que no se repitan pinones con el cambio
            ...
            if normrAux < bici.normr  % actualizamos la estructura si la norma ha mejorado
